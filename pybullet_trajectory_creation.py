...
def move(cartesian_destination, upper_limits, lower_limits, sawyerId, joints):
	...
	target_joint_state = np.array(p.calculateInverseKinematics(sawyerId, 18, cartesian_destination, lowerLimits=lower_limits, upperLimits = upper_limits))

	curJointPos = []
	for joint in joints:
		curJointPos.append(p.getJointState(sawyerId, joint)[0])

	velocities = get_control(target_joint_state, curJointPos) 

	p.setJointMotorControlArray(sawyerId, joints, p.POSITION_CONTROL, target_joint_state, velocities, [100, 100, 100, 100, 100, 100, 100, 100,100, 100])
	return
